...
parser.read(sys.path[0] + "/config.ini")
access_token = parser.get("config", "access_token")
zeno = zenodopy.Client(ACCESS_TOKEN=access_token)
...
for i, data in enumerate([metadata, properties, representations, callgraphs]):
    for project_identifier, filename in data.items():
        ...
        zeno.set_project(project_identifier)
        zeno.download_file(filename=filename, dst_path=dst_path)
